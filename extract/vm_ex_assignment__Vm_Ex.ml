  ...
  begin
    ...
    begin try
      ...
      | (_, instr :: _) ->
        ...
        | Vm__Vm.Ibeq ofs ->
          let (n2, rs23) = pop s in let (n1, rs13) = pop rs23 in
          begin match Z.equal n1 n2 with
          | true -> Vm__Vm.VMS ((Z.add (Z.add p Z.one) ofs), r, rs13, m)
          | false -> Vm__Vm.VMS ((Z.add p Z.one), r, rs13, m)
          end
        | Vm__Vm.Ibne ofs ->
          let (n21, rs24) = pop s in let (n11, rs14) = pop rs24 in
